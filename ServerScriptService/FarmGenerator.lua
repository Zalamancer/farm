...
local function createFarmForPlayer(player)
	if Workspace:FindFirstChild(player.Name .. "_Farm") then return end
	...
	local farmGridX = (player.UserId % 100) * FARM_SPACING
	local farmGridZ = math.floor(player.UserId / 100) * FARM_SPACING
	local startPosition = Vector3.new(farmGridX, 10.5, farmGridZ)
	...
	-- >> FIX: Use ExpandToGrid() to ensure the region is aligned before replacing the material.
	...
	if playerData.farm_objects then
		for id, data in pairs(playerData.farm_objects) do
			if not (data.CFrame and data.Template) then continue end

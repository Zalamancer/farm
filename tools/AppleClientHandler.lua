...
-- This function draws the green boxes over the valid fields
...
-- This function runs every frame to update the outline
...
-- >> FIX: The logic here is now more intentional.
tool.Activated:Connect(function()
	local target = mouse.Target

	-- Helper function to check if the clicked part is part of the player's farm
	local function isTargetOnFarm(part)
		if not part or not part.Parent then return false end
		local current = part
		for i = 1, 10 do -- Search up 10 levels to be safe
	...
	-- If the player clicks, but NOT on any part of their own farm, then eat.
